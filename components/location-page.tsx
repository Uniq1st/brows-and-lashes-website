...
import { GIFT_CARD_URL } from "@/lib/stores"
...
export function LocationPage({ store, teamMembers, heroTagline, neighborhoodDesc }: LocationPageProps) {
  return (
    <>
      <Navigation />
      <main className="pt-20">
          ...
          <div className="max-w-5xl mx-auto px-6">
            <div className="grid sm:grid-cols-2 md:grid-cols-4 gap-8 text-center">
              ...
              <div>
                ...
                <a href={store.phoneHref} className="font-[family-name:var(--font-montserrat)] text-sm text-muted-foreground font-light hover:text-primary transition-colors">
                  {store.phone}
                </a>
              </div>
              <div>
        ...
        <section className="py-20 bg-background">
          <div className="max-w-5xl mx-auto px-6">
            <div className="text-center mb-12">
              <p className="font-[family-name:var(--font-montserrat)] text-sm tracking-[0.3em] uppercase text-primary mb-4">Services</p>
              <h2 className="text-4xl font-light">Everything we offer<span className="block italic">at this studio</span></h2>
            </div>
            <div className="grid sm:grid-cols-2 md:grid-cols-3 gap-3 mb-10">
              {services.map((service) => (
                <div key={service} className="flex items-center gap-3 p-4 border border-border hover:border-primary/40 transition-colors">
                  <div className="w-1.5 h-1.5 rounded-full bg-primary shrink-0" />
                  <span className="font-[family-name:var(--font-montserrat)] text-sm">{service}</span>
                </div>
              ))}
            </div>
        ...
        {teamMembers && teamMembers.length > 0 && (
          <section className="py-20 bg-card">
            <div className="max-w-5xl mx-auto px-6">
              <div className="text-center mb-12">
                <p className="font-[family-name:var(--font-montserrat)] text-sm tracking-[0.3em] uppercase text-primary mb-4">Your Specialists</p>
                <h2 className="text-4xl font-light">Meet the team<span className="block italic">at this studio</span></h2>
              </div>
              <div className="grid sm:grid-cols-2 md:grid-cols-4 gap-6 text-center">
                {teamMembers.map((member) => (
                  <div key={member.name} className="bg-background border border-border p-6">
                    <div className={`w-16 h-16 rounded-full ${member.color} flex items-center justify-center mx-auto mb-4 text-lg font-semibold`}>
                      {member.initials}
                    </div>
                    <p className="font-medium mb-1">{member.name}</p>
                    <p className="font-[family-name:var(--font-montserrat)] text-xs text-muted-foreground tracking-wider uppercase">{member.role}</p>
                  </div>
                ))}
              </div>
            </div>
        ...
        <section className="py-20 bg-background">
          <div className="max-w-5xl mx-auto px-6">
            <div className="text-center mb-12">
              <p className="font-[family-name:var(--font-montserrat)] text-sm tracking-[0.3em] uppercase text-primary mb-4">Find Us</p>
              <h2 className="text-4xl font-light">{store.address}</h2>
              <p className="font-[family-name:var(--font-montserrat)] text-muted-foreground mt-2">{store.cityStateZip}</p>
            </div>
